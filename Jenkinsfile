...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="jamerl1210/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/Jamerl-1210/geustbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no root@3.36.94.26 docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no root@3.36.94.26 docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
                                        -e MYSQL_IP=3.39.179.72 \
                                        -e MYSQL_PORT=3306 \
